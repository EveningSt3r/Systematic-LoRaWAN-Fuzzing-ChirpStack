...
def get_redis_stream_length(stream_key):
    ...
    """
    stdout, _ = run_docker_cmd([ "docker", "exec", REDIS_CONTAINER,
...
def send_confirmedDataUp_1A():
    ...
    print("Part 1 of Phase 1: We send a confirmedDataUp frame using our real devAddr and a frozen FCnt" \
...
def valid_join_req_MIC_2():
    ...
    print(f"Current redis stream length: {redis_stream_length_snapshot}")   
    print(f"Current device state: {device_state_snapshot}")
    print("Phase 2: We have tested how Chirpstack reacts to an unjoined state. In order to transform to a joined state without manual"
...
def replay_then_oversized_3():
    ...
    send_packet(valid_header(), body)
    time.sleep(2)
    poll_redis_for_new_entry('gw:stream:frame', redis_stream_length_snapshot_2, timeout=5)
    print(get_recent_bridge_logs())
    print(get_device_state())
    print(f"[REPLAY] Iteration {x+1}/10")
...
def rejoinframe_new_session_4():
    ...
    if "NO SESSION" in state:
        ...
        return 0
    redis_stream_length_snapshot = get_redis_stream_length('gw:stream:frame')
    print("Part 1 of Phase 4: We send a rejoin request frame to trigger a new session. Then immediately a new data frame is sent" \
    ...
    print("Part 2 of Phase 4: Old frame sent. This part needs to happen immediately after")
    redis_stream_length_snapshot_2 = get_redis_stream_length('gw:stream:frame')
    frame2 = bytearray(19)
    frame2[0] = 0x80 # 1
    frame2[1:5] = bytes.fromhex("36d3fd00") # 2-5
...
if __name__ == "__main__":
    ...
    print(get_device_state())
    print(f"[TEST] Redis gw stream length: {get_redis_stream_length('gw:stream:frame')}")
    ...
    input("In the joined state. Replay attack and oversized payload. See logs then press enter.")
